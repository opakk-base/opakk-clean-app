...
async function walkDirForLargeFiles(rootDir, minBytes, options = {}) {
  ...
  const ignoreNames = /* @__PURE__ */ new Set(["node_modules", ".git", ".next", "dist", "build"]);
  const allowExt = new Set(
    extensions.map((e) => e.trim().toLowerCase()).filter(Boolean).map((e) => e.startsWith(".") ? e : `.${e}`)
  );
  const results = [];
  const folderStats = /* @__PURE__ */ new Map();
  ...
  const topFolders = [...folderStats.entries()].map(([folder, size]) => ({ folder, size, sizeText: bytesToReadable(size) })).sort((a, b) => b.size - a.size).slice(0, 15);
  return { files: results, topFolders };
}
electron.ipcMain.handle("app-platform", async () => process.platform);
electron.ipcMain.handle("app-home", async () => os.homedir());
electron.ipcMain.handle("scan-junk", async (_event, customTargets = []) => {
...
electron.ipcMain.handle("clean-junk-target", async (_event, targetPath, dryRun = false) => {
...
electron.ipcMain.handle("scan-large-files", async (_event, { rootDir, minMB = 100, maxResults = 500, maxDepth = 10, extensions = [] }) => {
...
electron.ipcMain.handle("delete-paths", async (_event, paths = [], dryRun = false) => {
  if (dryRun) {
